...
const (
	...
	// tbdProject is the project name on Docker Hub
	tbdProject = "ghcr.io/jphastings/tbd"
...
func run(docker *client.Client, target, mainPath string, mounts []mount.Mount) error {
	...
	if err != nil {
		return fmt.Errorf("couldn't create a tbd container\n%w", err)
	}

	if err := docker.ContainerStart(ctx, c.ID, types.ContainerStartOptions{}); err != nil {
		return fmt.Errorf("couldn't start the tbd container\n%w", err)
	}
...
func exitWithUsage() {
	...
	fmt.Printf("Usage: %s <target-device> [output-path [path-to-main-package]]\n"+
		"Builds a go app for a System-on-Chip device.\n\n"+
		"eg. %s rpi /Volumes/sd-card .\n",
		app, app)
	os.Exit(1)
}
